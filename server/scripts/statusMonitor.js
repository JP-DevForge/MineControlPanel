...
function getServersWithSamePort(server, servers) {
  ...
  return servers.filter(
    item => getServerPort(item) === port
  );
}

async function identifyServerByRcon(
  port,
  samePortServers
) {
  const alreadyIdentified =
    identifiedPortServer.get(port);
...
for (const server of samePortServers) {
  try {
    await minecraft.checkRconConnection(server);

    identifiedPortServer.set(
      port,
      server.id
    );
...
async function detectServerStatus(
  ...
  const samePortServers =
    getServersWithSamePort(
      server,
      servers
    );
  ...
  if (!pingData) {
    const failures =
      (failedPingCount.get(server.id) || 0) + 1;
...
/*
=========================================
JUGADORES (DESACTIVADO)
=========================================

const {
  syncPlayers,
  refreshOnlinePlayers
} = require("./syncjson/PlayersJSON");

const lastPlayerCount = new Map();
const lastPlayersRefresh = new Map();

const PLAYERS_REFRESH_INTERVAL = 3000;

const onlineCount = current.players?.online;

if (current.online && typeof onlineCount === "number") {
  const previousCount =
    lastPlayerCount.get(server.id);

  const now = Date.now();

  const lastRefresh =
    lastPlayersRefresh.get(server.id) || 0;

  try {
    if (previousCount !== onlineCount) {
      await syncPlayers(server);
      ...
      await refreshOnlinePlayers(server);
    ...
    console.error(
      `[${server.name}] Error actualizando players.json: ${error.message}`
    );
  }
}
*/